...
function gameRound(playerSelection, computerSelection) {
    ...
    if (winningCond[computerSelection] === playerSelection) {
        return [`You lose! ${computerSelection} beats ${playerSelection}`, -1]; // -1 player
        // lose
    }
    if ((winningCond[computerSelection] !== playerSelection) && (Object.keys(winningCond).includes(playerSelection)
        && (Object.keys(winningCond).includes(playerSelection)))) {
...
function game(ObjectIn, resultsDiv) {
    const roundResult = gameRound(ObjectIn.target.textContent.trim(), getComputerChoice());
    if (roundResult[1] === -1) {
        computerScore += 1;
